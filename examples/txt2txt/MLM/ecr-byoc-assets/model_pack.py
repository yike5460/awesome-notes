...
def download_and_package_model(model_id, platform, ecr_repo, model_bucket):
    ...
    """
    # TODO, update the model.py accordingly
    # Copy model files
    COPY model.py requirements.txt /opt/ml/code/

    # Copy custom metric collection script 
    COPY collect_gpu_metrics.py /opt/ml/code/
    """

    # Build and push the Docker image
    os.system(f"docker build -t {ecr_repo}:latest .")
